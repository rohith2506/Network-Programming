...
int main(){
    ...
    bzero(&srvr2,sizeof(struct sockaddr_un));
    srvr2.sun_family = AF_UNIX;
    strcpy(srvr2.sun_path,"c1");
    
    if((fd2=socket(AF_UNIX,SOCK_STREAM,0))<0)
        perror("Socket:\n");
    
    size = sizeof(srvr2.sun_family)+strlen(srvr2.sun_path);

    if(connect(fd2,(struct sockaddr *)&srvr2,size)==-1)
        perror("connect:\n");		
    char ch[1];
    read(fd2,ch,1);
    if(ch[0]=='Y')
    ...
    while(1){
    char buf[100];
    int n = read(fd3,buf,100);
    printf("Message recevied from server:\n");
    write(1,buf,n);
    }
    }
    else
